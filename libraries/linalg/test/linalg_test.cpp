...
CIE_TEST_CASE( "Matrix", "[matrix]" )
{
    ...
    CIE_TEST_CHECK( Matrix<Double>( 1, 3, 0.0 ).size1( ) == 1 );
    CIE_TEST_CHECK( Matrix<Double>( 1, 3, 0.0 ).size2( ) == 3 );
    CIE_TEST_CHECK( Matrix<Double>( 5, 2 ).sizes( )[0] == 5);
    CIE_TEST_CHECK( Matrix<Double>( 5, 2 ).sizes( )[1] == 2 );

    Matrix<Double> matrix1( 2, 2, 4.2 );
    Matrix<Double> matrix( matrix1 );

    CIE_TEST_REQUIRE( matrix.size1( ) == 2 );
    CIE_TEST_REQUIRE( matrix.size2( ) == 2 );
    CIE_TEST_REQUIRE( matrix.sizes( )[0] == 2 );
    CIE_TEST_REQUIRE( matrix.sizes( )[1] == 2 );

    for( Size i = 0; i < 2; ++i )
    {
        for( Size j = 0; j < 2; ++j )
        {
            CIE_TEST_CHECK( matrix( i, j ) == 4.2 );
    ...
    for( Size i = 0; i < 2; ++i )
    {
        for( Size j = 0; j < 2; ++j )
        {
            CIE_TEST_CHECK( ref( i, j ) == i * 10.0 + j );
        }
    }
}
...
CIE_TEST_CASE( "zero size Matrix", "[matrix]" )
{
    ...
    CIE_TEST_REQUIRE_NOTHROW( m = Matrix<Double>( 0, 0, 2.0 ) );

    CIE_TEST_CHECK( m.size1( ) == 0 );
    CIE_TEST_CHECK( m.size2( ) == 0 );

    CIE_TEST_REQUIRE_NOTHROW( m = Matrix<Double>{ std::vector<DoubleVector>{ } } );
...
CIE_TEST_CASE( "inconsistent input to Matrix", "[matrix]" )
{
    ...
    CIE_TEST_CHECK_THROWS_AS( Matrix<Double>{ inconsistentData }, OutOfRangeException );

    CIE_TEST_CHECK_NOTHROW( Matrix<Double>{ { DoubleVector { } } } );

    CIE_TEST_CHECK( Matrix<Double>{ { DoubleVector { } } }.size1( ) == 1 );
    CIE_TEST_CHECK( Matrix<Double>{ { DoubleVector { } } }.size2( ) == 0 );
}
...
CIE_TEST_CASE( "linearized input to Matrix", "[matrix]" )
{
    ...
    CIE_TEST_CHECK_THROWS_AS( Matrix<Double>( data, 2 ), OutOfRangeException );

    data.pop_back( );

    CIE_TEST_REQUIRE_NOTHROW( Matrix<Double>( data, 2 ) );

    Matrix<Double> m( data, 2 );

    CIE_TEST_CHECK( m( 0, 0 ) == 0.0 );
    CIE_TEST_CHECK( m( 0, 1 ) == 1.0 );
    CIE_TEST_CHECK( m( 0, 2 ) == 2.0 );
    CIE_TEST_CHECK( m( 1, 0 ) == 3.0 );
    CIE_TEST_CHECK( m( 1, 1 ) == 4.0 );
    CIE_TEST_CHECK( m( 1, 2 ) == 5.0 );
}
...
namespace linalgtesthelper
{
    ...
    std::vector<Double> writeAndParse( const ContainerType& container )
    {
        ...
        CIE_TEST_REQUIRE_NOTHROW( linalghelper::write( container, outfile ) );
...
CIE_TEST_CASE( "write vector", "[io]" )
{
    ...
    CIE_TEST_CHECK( result[0] == v[0] );
    CIE_TEST_CHECK( result[1] == v[1] );
    CIE_TEST_CHECK( result[2] == v[2] );
}
...
CIE_TEST_CASE( "write matrix", "[io]" )
{
    ...
    CIE_TEST_REQUIRE( result.size( ) == 4 );

    CIE_TEST_CHECK( result[0] == m( 0, 0 ) );
    CIE_TEST_CHECK( result[1] == m( 0, 1 ) );
    CIE_TEST_CHECK( result[2] == m( 1, 0 ) );
    CIE_TEST_CHECK( result[3] == m( 1, 1 ) );
}
...
CIE_TEST_CASE( "norm", "[utility]" )
{
    ...
    CIE_TEST_CHECK( norm( vector ) == Approx( 5.0 ).epsilon( tolerance ) );

    Matrix<Double> matrix( { DoubleVector{  1.2,  3.3,  9.1 },
                     DoubleVector{  2.9,  8.6,  2.6 },
                     DoubleVector{ -4.8,  0.3, -2.1 } } );

    CIE_TEST_CHECK( norm( matrix ) == Approx( 14.553693689232297 ).epsilon( tolerance ) );
}
...
CIE_TEST_CASE( "solve", "[solvers]" )
{
    ...
    CIE_TEST_REQUIRE( matrix.size1( ) == 4 );
    CIE_TEST_REQUIRE( matrix.size2( ) == 4 );

    DoubleVector solution = solve( matrix, rhs );

    CIE_TEST_REQUIRE( solution.size( ) == 4 );

    Double tolerance = 1e-12;

    CIE_TEST_CHECK( solution[0] == Approx(  1.3218868527560599 ).epsilon( tolerance ) );
    CIE_TEST_CHECK( solution[1] == Approx(  1.4702413909379624 ).epsilon( tolerance ) );
    CIE_TEST_CHECK( solution[2] == Approx( -1.7634954348963456 ).epsilon( tolerance ) );
    CIE_TEST_CHECK( solution[3] == Approx( -0.1893110309940256 ).epsilon( tolerance ) );
}
...
CIE_TEST_CASE( "solve singular", "[solvers]" )
{
    ...
    CIE_TEST_CHECK_THROWS_AS( solve( matrix, rhs ), MatrixError );
}
...
CIE_TEST_CASE( "solve zero pivot", "[solvers]" )
{
    ...
    CIE_TEST_REQUIRE_NOTHROW( computedSolution = solve( matrix, rhs ) );
    CIE_TEST_REQUIRE( computedSolution.size( ) == 4 );

    Double tolerance = 1e-12;

    for( Size i = 0; i < 4; ++i )
    {
    	CIE_TEST_CHECK( computedSolution[i] == Approx( expectedSolution[i] ).epsilon( tolerance ) );
    }
}
